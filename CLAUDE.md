...
`tabs.py` + `tabsbody.py` + `tabsend.py` + `ghostty.py` + `transcript.py` own a lane (peer to triggers / captures / sessions / mailbox) that mirrors **live Ghostty tabs running Claude Code** into the `Claude: Tabs` list (override `RBRIDGE_TABS_LIST`; deliberately drops the `Beads: ` prefix). `tabs.sync()` runs once per cycle from `daemon.sync_once`. Discovery learned from the `node/claude-activity-watcher` tool.

- **Discovery** (`ghostty.discover()`): parse `ps -axo pid=,ppid=,tty=,command=`, find the Ghostty root pid(s) (command contains `Ghostty.app/Contents/MacOS/ghostty`), then keep every process whose argv0 basename is `claude`, has a real tty, is not `claude daemon …`, and whose ppid chain reaches a Ghostty root. cwd via `lsof -p <pid> -a -d cwd -F n`; one `Tab` per process. **Important:** pass `env={**os.environ, "LANG": "C"}` — wiping the env drops `PATH` and `lsof` (in `/usr/sbin`) won't resolve.
- **Session resolution** (`transcript.resolve_session`): newest `*.jsonl` under `~/.claude/projects/<encoded-cwd>/`, where `encode_cwd` replaces `/`, `.`, and `_` with `-` (verified against the live dir — dotted segments like `.claude-worktrees` encode to `--claude-...`; the activity-watcher's `[/_]`-only regex is wrong for dotted paths). Tabs sharing a cwd share the newest jsonl — accepted limitation.
- **Reminder per tab** keyed by pid (`priority 1`, title `{project} · {tty}`). Body is daemon-owned and overwritten each cycle: header (`project` / `cwd` / `tty`·`pid`·`mode` / `session`), a live transcript tail (last `RBRIDGE_TABS_TAIL_MSGS`, default 6), a `messages` log, and a trailing `send:` region. `tabsbody.py` owns all body string concerns (peer to `body.py`); `tabsbody.parse_send` is the **only** thing read back from the user.
- **Send (R→fork, the lane's one write-back signal):** type under `send:` then **check the reminder** — completion is the explicit send trigger (avoids firing on half-typed text). `tabs.sync` reads the payload, calls `tabsend.launch` (`claude -p --output-format json --resume <sid>`), appends `you (ts): …`, and unchecks the reminder. `tabsend.collect()` reaps finished turns next cycle and `tabs._apply_replies` appends `claude (ts): …`. The first send resumes the tab's **live** session id; the reply's **forked** session id (from the `result` event) is stored as `fork_sid` and reused for follow-ups, so the bridge never writes the jsonl the live tab is appending to. It does **not** type into the live tab. Timeout `RBRIDGE_TABS_TIMEOUT_S` (default 900s).
- **GC:** a tab whose pid is gone → delete its reminder + state (`tab-closed`). This lane is a live mirror, not a chat store. State: `~/.claude/reminders-bridge-tabs.json` (per-tab reminder id / `fork_sid` / messages) + `~/.claude/reminders-bridge-tab-turns.json` (in-flight turns). Activity events: `tab-opened`, `tab-send`, `tab-reply`, `tab-closed`. CLI: `rbridge tabs` (read-only listing); `rbridge doctor` reports the live tab count.
