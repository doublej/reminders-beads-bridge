...
- **claude tabs** (`tabs.py`, `tabsbody.py`, `ghostty.py`, `transcript.py`, `inject.py`) — mirror live Ghostty Claude tabs into `_rb_claude_tabs`; `send:` + check types into the live tab (GUI inject, needs Accessibility). See `docs/REFERENCE.md` → "Claude tabs".
